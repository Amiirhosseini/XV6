...
int pssyscall()
{
  ...
  cprintf("pid    state    name         ppid      siblings\n");
  ...
  for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
  {
    ...
    if (p->state >= 0 && p->state < NELEM(states) && states[p->state])
      ...
      state = "???";
    cprintf(" %d     %s     %s        %d       ", p->pid, state, p->name, p->parent->pid);

    struct proc *j;
    int has_Sibling = 0;

    for (j = ptable.proc; j < &ptable.proc[NPROC]; j++) //find siblings
    {
      if (j->parent->pid == p->parent->pid && j->parent->pid!=0 &&  j->pid!=0 && j->parent->pid != j->pid && p->parent->pid != p->pid && p->pid != j->pid)
      {
        cprintf("%d,", j->pid);
        has_Sibling = 1;
      }
